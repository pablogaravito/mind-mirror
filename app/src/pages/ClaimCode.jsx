...
export default function ClaimCode({ profile, session }) {
  ...
  async function handleSubmit(e) {
    ...
    const { error: claimErr } = await supabase
      ...
      .eq('id', assignment.id)
      .is('user_id', null) // safety: only claim if still unclaimed

    if (claimErr) {
      setError('No se pudo canjear el código. Intenta de nuevo.')
      setLoading(false)
      return
    }
  ...
  if (checking) return <div className="spinner" style={{ marginTop: '6rem' }} />
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
      <div style={{ width: '100%', maxWidth: '420px' }}>
        ...
        <div className="text-center" style={{ marginBottom: '2rem' }}>
          ...
          <p className="mt-1">
            Tu psicólogo te habrá enviado un código de acceso. Ingrésalo a continuación para ver tus evaluaciones.
          </p>
        </div>
        ...
        <div className="card">
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            <button className="btn btn--primary btn--full" type="submit" disabled={loading || !code.trim()}>
