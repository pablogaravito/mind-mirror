...
function TestsTab() {
  ...
  async function updateTest(testId, field, value) {
    ...
    if (field === 'is_active') {
      ...
      // Merge into existing report_config rather than replacing it
      update = { report_config: { ...test.report_config, [field]: value } }
    }
    ...
    if (!error) {
      setTests(prev => prev.map(t => {
        ...
        return { ...t, report_config: { ...t.report_config, [field]: value } }
      }))
    } else {
...
function AssignmentCard({ assignment, onUpdate }) {
  ...
  async function openAddTest() {
    ...
    const { data: allTests } = await supabase
      ...
      .select('id, name')
      .eq('is_active', true)
    const assignedIds = (assignment.assigned_tests || []).map(at => at.test_id)
    setAvailableTests((allTests || []).filter(t => !assignedIds.includes(t.id)))
    setSelectedTest('')
  ...
  async function handleAddTest() {
    if (!selectedTest) { setAddError('Selecciona un test.'); return }
    setAdding(true)
    ...
    setAdding(false)
    if (error) { setAddError('Error al agregar el test.'); return }
          ...
          <div style={{ marginTop: '0.5rem', display: 'flex', flexWrap: 'wrap', gap: '0.4rem', alignItems: 'center' }}>
            ...
            {assignment.status !== 'completed' && (
              <button
                ...
                style={{ fontSize: '0.75rem', padding: '0.15rem 0.5rem', color: 'var(--accent)' }}
                onClick={openAddTest}
              >
                + Agregar test
              </button>
          ...
          {showAddTest && (
            <div style={{
              ...
              {availableTests.length === 0 ? (
                <p style={{ fontSize: '0.875rem' }}>No hay tests adicionales disponibles.</p>
              ) : (
                <>
                  <div className="field">
                  ...
                  {addError && <p style={{ color: 'var(--danger)', fontSize: '0.8rem' }}>{addError}</p>}
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    <button className="btn btn--primary" style={{ fontSize: '0.875rem' }} onClick={handleAddTest} disabled={adding}>
                      {adding ? 'Agregando...' : 'Agregar'}
                    </button>
                    <button className="btn btn--ghost" style={{ fontSize: '0.875rem' }} onClick={() => setShowAddTest(false)}>
                      Cancelar
                    </button>
                  </div>
                </>
