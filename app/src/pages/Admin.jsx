  ...
  useEffect(() => {
    async function load() {
      const { data } = await supabase
        .from("test_sessions")
        .select(
          "id, completed_at, scores, status, tests(name, slug), profiles:user_id(full_name)",
        )
        .eq("status", "completed")
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
      {sessions.map((sess) => {
        ...
        const domains = Object.values(scores).filter(
          (s) => s.type === "domain",
        );
  ...
  async function load() {
    const [{ data: aData }, { data: tData }] = await Promise.all([
      supabase
        .from("assignments")
        .select(
          "*, assigned_tests(*, tests(name)), profiles:user_id(full_name)",
        )
...
function NewAssignmentForm({ tests, onClose, onCreated }) {
  ...
  async function handleCreate() {
    ...
    if (!selectedTests.length) {
      setError("Asigna al menos un test.");
      return;
    }
    setLoading(true);
    setError("");
    ...
    const { error: tErr } = await supabase
      ...
      .insert(testRows);
    if (tErr) {
      setError("Error al asignar tests.");
      setLoading(false);
      return;
    }
  ...
  return (
    ...
    >
      <h3 style={{ color: "var(--text)", marginBottom: "1.25rem" }}>
        Nueva asignación
      </h3>
      <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
        <div className="field">
        ...
        <div className="field">
          ...
          <input
            placeholder="Ej. Derivado por Dr. Martínez"
            value={notes}
              ...
              return (
                ...
                >
                  ...
                  {selected && (
                    <select
                      value={
                        selected.show_results === null
                          ? "default"
                          : selected.show_results
                            ? "yes"
                            : "no"
                      }
                      onChange={(e) =>
                    ...
                    >
                      <option value="default">
                        Visibilidad: por defecto del test
                      </option>
...
function AssignmentCard({ assignment, onUpdate }) {
  ...
  const [showResults, setShowResults] = useState("default");
  ...
  async function openAddTest() {
    // Load tests not already assigned
    const { data: allTests } = await supabase
      .from("tests")
      .select("id, name")
      .eq("is_active", true);
    const assignedIds = (assignment.assigned_tests || []).map(
      (at) => at.test_id,
    );
    setAvailableTests(
      (allTests || []).filter((t) => !assignedIds.includes(t.id)),
    );
    setSelectedTest("");
    setShowResults("default");
    setAddError("");
    setShowAddTest(true);
  }
  ...
  async function handleAddTest() {
    if (!selectedTest) {
      setAddError("Selecciona un test.");
      return;
    }
    setAdding(true);
    const nextOrder = (assignment.assigned_tests || []).length;
    const { error } = await supabase.from("assigned_tests").insert({
      assignment_id: assignment.id,
      test_id: parseInt(selectedTest),
      display_order: nextOrder,
      show_results: showResults === "default" ? null : showResults === "yes",
      is_active: true,
    });
    setAdding(false);
    if (error) {
      setAddError("Error al agregar el test.");
      return;
    }
      ...
      >
        <div style={{ flex: 1 }}>
          ...
          >
            ...
            <span className={`badge ${statusLabel.cls}`}>
              {statusLabel.label}
            </span>
          </div>
          ...
          >
            ...
            {/* Add test button — always visible for claimed/pending */}
            ...
            >
              {availableTests.length === 0 ? (
                <p style={{ fontSize: "0.875rem" }}>
                  No hay tests adicionales disponibles.
                </p>
              ) : (
                <>
                  <div className="field">
                    <label>Test a agregar</label>
                    <select
                      value={selectedTest}
                  ...
                  <div className="field">
                    ...
                    >
                      <option value="default">Por defecto del test</option>
                      <option value="yes">Mostrar al usuario</option>
                      <option value="no">Ocultar al usuario</option>
                    </select>
                  </div>
                  {addError && (
...
function PdfRequestsTab() {
  ...
  async function loadRequests() {
    ...
    const { data } = await supabase
      .from("pdf_requests")
      .select(
        "*, profiles:user_id(full_name), test_sessions:session_id(completed_at, tests(name))",
      )
      .eq("status", filter)
    ...
    <div>
      ...
      {!loading && requests.length === 0 && (
        <div className="card text-center" style={{ padding: "3rem" }}>
          <p>
            No hay solicitudes{" "}
            {filterOptions.find((f) => f.value === filter)?.label.toLowerCase()}
            .
          </p>
        </div>
      )}
