...
export default function AdminSessionView() {
  ...
  async function load() {
    ...
    if (err || !data) { setError('Sesión no encontrada.'); setLoading(false); return }
    setSess(data)
    ...
    const { data: respData } = await supabase
      .from('test_responses')
      .select('response_value, answered_at, test_items(item_number, text, is_reversed, aspect)')
  ...
  const completedDate = sess.completed_at
    ? new Date(sess.completed_at).toLocaleDateString('es-PE', { day: 'numeric', month: 'long', year: 'numeric' })
    : '—'
      ...
      <div className="container">
        ...
        <div style={{ marginBottom: '2rem' }}>
          <button
            ...
            style={{ padding: 0, marginBottom: '0.75rem', fontSize: '0.875rem', color: 'var(--text-muted)' }}
          ...
          <div style={{
            ...
            <span><strong style={{ color: 'var(--text)' }}>Persona:</strong> {profile?.full_name || '—'}</span>
            <span><strong style={{ color: 'var(--text)' }}>Fecha de nacimiento:</strong> {profile?.birth_date || '—'}</span>
            <span><strong style={{ color: 'var(--text)' }}>Género:</strong> {profile?.gender || '—'}</span>
            <span><strong style={{ color: 'var(--text)' }}>Completado:</strong> {completedDate}</span>
          </div>
        </div>
        ...
        {responses.length > 0 && (
          <div style={{ marginTop: '2rem' }}>
            ...
            >
              {showResponses ? 'Ocultar respuestas individuales ↑' : 'Ver respuestas individuales ↓'}
            </button>
            ...
            {showResponses && (
              <div className="card" style={{ padding: '1.25rem' }}>
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          alignItems: 'center',
                          padding: '0.6rem 0',
                          borderBottom: idx < responses.length - 1 ? '1px solid var(--border)' : 'none',
                          fontSize: '0.875rem',
                        }}
                      >
                ...
                <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: '0.75rem' }}>
                  ® indica ítem inverso (el valor fue invertido para el puntaje).
                </p>
              </div>
            )}
          </div>
