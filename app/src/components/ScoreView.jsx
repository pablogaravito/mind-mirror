      ...
      {domain.description && (
        <p
          style={{
            ...
            marginBottom: interp?.content ? "0.5rem" : 0,
            paddingLeft: "1.25rem",
          }}
        >
          {domain.description}
          ...
          {aspect.description && (
            <p
              style={{
                fontSize: "0.85rem",
                color: "var(--text-muted)",
                marginBottom: interp?.content ? "0.4rem" : 0,
              }}
            >
              {aspect.description}
            </p>
...
export function InterpretationText({ blocks, scale, aspect, score }) {
  ...
  return blocks.map((block, i) => {
    if (block.type === "percentile_inject") {
      ...
      return (
        <p key={i} style={{ marginTop: "0.75rem" }}>
          Tu puntaje te posiciona en el percentil <strong>{pct}</strong> de{" "}
          {scaleName}. Esto significa que, si fueras una de 100 personas en una
          habitación, tendrías {pct > 50 ? "mayor" : "menor"} {scaleName} que{" "}
    ...
    return block.content.split("\n\n").map((para, j) => (
      <p key={`${i}-${j}`} style={{ marginTop: j > 0 ? "0.75rem" : 0 }}>
        {para}
      </p>
    ));
  });
